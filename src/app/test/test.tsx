...
const TestFunctions = ({ session }) => {
  ...
  const get_my_claims = async () => {
    ...
    setNotes(
      'This calls the server function "get_my_claims()" and gets the claims from the current token at the server.'
    );
  };
  ...
  const show_session = async () => {
    ...
    setNotes(
      'This displays the entire session object that was returned from "supabase.auth.onAuthStateChange".'
    );
  };
  const is_claims_admin = async () => {
    ...
    setNotes(
      'This calls the server function "is_claims_admin()" and returns true if the current token on teh server has the "claims_admin" claim.'
    );
  };
  const session_claims = async () => {
    ...
    setNotes(
      'This returns the value of "app_metadata" (the claims) from the current session object (returned from "supabase.auth.onAuthStateChange").'
    );
  };
  const set_claim = async () => {
    ...
    setNotes(
      'This calls the server function "set_claim(uid, claim, value)" to set a custom claim for a given user by id (uuid).'
    );
  };
  const delete_claim = async () => {
    ...
    setNotes(
      'This calls the server function "delete_claim(uid, claim)" to delete a custom claim for a given user by id (uuid).'
    );
  };
  ...
  return (
    <>
      <div className="center">
        Local:
        <button onClick={show_session}>session</button>
        <button onClick={session_claims}>session_claims</button>
        <button onClick={refresh_claims}>refresh_claims</button>
      </div>
      <div className="center">
        Server:
        <button onClick={get_my_claims}>get_my_claims()</button>
        <button onClick={is_claims_admin}>is_claims_admin()</button>
        claim:{" "}
        <input
          type="text"
          value={claim}
        ...
        />
        <button onClick={get_my_claim}>get_my_claim('{claim}')</button>
      </div>
      <div className="center">
        uid:{" "}
        <input
        ...
        <button
          onClick={delete_claim}
          disabled={
            !uid || !claim || !session?.user?.app_metadata?.claims_admin
          }
        >
          delete_claim
        </button>
      ...
      <div className="center">
        {session?.user?.app_metadata?.claims_admin && <span>ADMIN</span>}
        {!session?.user?.app_metadata?.claims_admin && <span>NOT ADMIN</span>}
      </div>
      <div>
      ...
      <div className="title">
        {session?.user?.app_metadata?.claims_admin
          ? "you are a CLAIMS_ADMIN"
          : "you are NOT a CLAIMS_ADMIN"}
      </div>
    </>
  );
};
